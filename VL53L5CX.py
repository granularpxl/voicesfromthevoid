...
while True:
    if vl53.data_ready():
        data = vl53.get_data()
        distanceraw = numpy.array(data.distance_mm, dtype=numpy.float64)[0, 4:12].reshape((2, 4))

        # 2d array of good ranging data
        status = numpy.isin(numpy.array(data.target_status)[0, 4:12].reshape((2, 4)), (STATUS_RANGE_VALID))
